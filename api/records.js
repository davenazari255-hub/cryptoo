// Returns the authenticated user's detailed deposit + withdrawal history from
// the server (source of truth). TG-authed. Self-contained for Vercel.
...
module.exports = async function handler(req, res) {
  ...
  try {
    const [wdIds, ledger] = await Promise.all([
      upstash(['LRANGE', `wd:user:${userId}`, 0, 49]),
      upstash(['LRANGE', `ledger:${userId}`, 0, 49]),
    ]);
    let withdrawals = [];
    ...
    const deposits = (ledger || []).map(parseJSON).filter(Boolean);
    return res.status(200).json({ withdrawals, deposits });
  } catch (err) {
